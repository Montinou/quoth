...
- Tests the full schema creation including all tables:
  - `patterns` — includes `pattern_type` (default `code-pattern`), `description`, `success_count`, `failure_count`, `decay_rate`, `embedding`, `version`, `tags`, `source`, `status`, `last_matched_at`
