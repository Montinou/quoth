      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 animate-stagger stagger-7">
          ...
          <Tabs value={filter} onValueChange={setFilter}>
            <TabsList className="bg-zinc-900/80 border border-zinc-800 h-auto flex-wrap gap-1 p-1.5">
              {filters.map((status) => {
                ...
                return (
                  <TabsTrigger
                    key={status}
                    value={status}
                    className="data-[state=active]:bg-violet-spectral data-[state=active]:text-white data-[state=active]:shadow-lg data-[state=active]:shadow-violet-spectral/20 text-gray-400 hover:text-white flex items-center gap-2 px-4 py-2"
                  >
                    {status !== 'all' && statusConfig[status] && (
                      <span className={`w-2 h-2 rounded-full ${statusConfig[status].dotColor}`} />
                    )}
                    <span>{status.charAt(0).toUpperCase() + status.slice(1)}</span>
                    <span className="px-1.5 py-0.5 text-xs rounded-md bg-zinc-800 data-[state=active]:bg-white/20">
                      {count}
                    </span>
                  </TabsTrigger>
                );
