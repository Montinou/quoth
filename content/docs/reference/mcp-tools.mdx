---
title: "MCP Tools Reference"
description: "Complete reference for all Quoth MCP tools"
order: 1
---

Quoth exposes several tools through the Model Context Protocol (MCP). These tools are called automatically by your AI assistant when relevant.

## Tools Overview

| Tool | Purpose | Auth Required |
|------|---------|---------------|
| `quoth_guidelines` | Get adaptive guidelines for coding/review/documentation | Yes |
| `quoth_search_index` | Semantic search across documentation | Yes |
| `quoth_read_doc` | Read full document content | Yes |
| `quoth_propose_update` | Submit documentation updates | Yes (editor+) |
| `quoth_list_templates` | List available templates | Yes |
| `quoth_get_template` | Fetch template structure | Yes |
| `quoth_read_chunks` | Fetch specific chunks | Yes |
| `quoth_project_list` | List all accessible projects | Yes |
| `quoth_project_read` | Get detailed project information | Yes |
| `quoth_project_create` | Create new project in your organization | Yes |
| `quoth_project_update` | Update project settings | Yes (admin) |
| `quoth_project_delete` | Permanently delete project | Yes (admin) |
| `quoth_genesis` | Bootstrap project documentation | Yes (admin) |

## quoth_guidelines

Get adaptive guidelines for your current task. **Strongly recommended** before writing code, reviewing, or documenting.

<Callout type="tip">
  This tool replaces the need to manually activate personas. Claude can call it autonomously when writing code.
</Callout>

### Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `mode` | string | Yes | `code`, `review`, or `document` |
| `full` | boolean | No | Return full guidelines (~500 tokens) instead of compact (~150 tokens) |

### Modes

| Mode | Use When | Returns |
|------|----------|---------|
| `code` | Writing or editing code | Patterns, anti-patterns, trust levels |
| `review` | Auditing existing code | Violation detection, drift vs new pattern |
| `document` | Creating/updating Quoth docs | Template requirements, embedding rules |

### Example

```
Get Quoth guidelines for writing code
→ quoth_guidelines({ mode: "code" })
```

### Response (Compact)

```markdown
## Quoth Guidelines: Code

**STRONGLY RECOMMENDED:** Call `quoth_search_index` before proceeding.

### Core Rules
1. Search first - `quoth_search_index` before coding
2. Trust levels: >80% follow exactly, 60-80% verify, <60% cross-ref
3. Docs = intended design - when code conflicts, docs win
4. Never invent patterns - only use what's documented
5. Templates required - `quoth_get_template` before Quoth updates

### Anti-Patterns
- Assuming patterns without searching
- Inventing patterns not in documentation
- Ignoring LOW trust warnings
```

## quoth_search_index

Performs semantic search across your knowledge base using AI embeddings.

### Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `query` | string | Yes | Natural language search query |
| `limit` | number | No | Max results (default: 15) |
| `rerank` | boolean | No | Enable Cohere reranking (default: true) |

### Example

```
Search Quoth for "authentication patterns"
```

Returns ranked results with relevance scores and content snippets.

## quoth_read_doc

Retrieves the full content of a specific document.

### Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `doc_id` | string | Yes | Document title or path |

### Example

```
Read the "testing-patterns" document from Quoth
```

## quoth_propose_update

Submits a documentation update proposal with evidence.

<Callout type="warning">
  Requires **editor** or **admin** role. Viewers cannot propose updates.
</Callout>

### Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `doc_id` | string | Yes | Document to update |
| `new_content` | string | Yes | Proposed new content (full markdown) |
| `evidence_snippet` | string | Yes | Code or commit reference |
| `reasoning` | string | Yes | Why this update is needed |

## Project Management Tools

Quoth provides a complete CRUD interface for managing projects via MCP.

## quoth_project_list

Lists all projects accessible to you, showing your role and the current active project.

### Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `organization_id` | string (UUID) | No | Filter projects by organization |

### Example

```javascript
// List all your projects
quoth_project_list()

// List projects in specific organization
quoth_project_list({ organization_id: "..." })
```

### Response

Shows:
- Project slug and ID
- Your role (admin/editor/viewer)
- Visibility (public/private)
- Creation date
- Active project indicator (✓)

## quoth_project_read

Get detailed information about a specific project.

### Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `project_id` | string (UUID) | No | Project ID (defaults to current active project) |

### Example

```javascript
// Get current project details
quoth_project_read()

// Get specific project details
quoth_project_read({ project_id: "..." })
```

### Response

Includes:
- Project settings (slug, visibility, GitHub repo, approval mode)
- Organization name
- Member count
- Document count
- Creation timestamp

## quoth_project_create

Creates a new project in your organization. Automatically assigns you as project admin.

<Callout type="info">
  If you don't have an organization yet, one will be created automatically when you create your first project.
</Callout>

### Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `name` | string | Yes | Human-readable project name (e.g., "My Knowledge Base") |
| `slug` | string | Yes | URL-safe identifier (lowercase, numbers, hyphens only) |
| `github_repo` | string | No | Optional GitHub repository (e.g., "owner/repo") |
| `is_public` | boolean | No | Make project publicly accessible (default: false) |

### Example

```javascript
// Create a new private project
quoth_project_create({
  name: "My Team Knowledge Base",
  slug: "my-team-kb",
  github_repo: "my-org/docs",
  is_public: false
})
```

### Response

Returns project details including:
- Project ID and slug
- Dashboard URL
- Next steps (indexing docs, inviting team, generating API token)

### Permissions

- ✅ Any authenticated user can create projects
- ✅ Project creator is automatically assigned as admin
- ❌ Slug must be globally unique

### After Creation

1. **Index Documentation:** Use `quoth_propose_update` to add documents
2. **Invite Team:** Go to Settings → Members in the dashboard
3. **Generate API Token:** Settings → API Tokens for MCP authentication
4. **Switch Context:** Use `quoth_switch_account` if managing multiple projects

## quoth_project_update

Updates project settings. Requires admin role.

### Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `project_id` | string (UUID) | No | Project ID (defaults to current active project) |
| `slug` | string | No | New URL-safe slug |
| `github_repo` | string | No | GitHub repository URL |
| `is_public` | boolean | No | Public visibility |
| `require_approval` | boolean | No | Require approval for doc updates |

### Example

```javascript
// Make project public
quoth_project_update({
  is_public: true
})

// Change slug and disable approval
quoth_project_update({
  slug: "new-slug",
  require_approval: false
})
```

### Permissions

- ❌ Requires admin role
- ✅ Can update current or any project you admin
- ✅ Slug must be globally unique

### Response

Shows updated settings and which fields were changed.

## quoth_project_delete

Permanently deletes a project and all associated data. **THIS CANNOT BE UNDONE.**

<Callout type="warning">
  **Irreversible Action:** All documents, embeddings, proposals, and memberships will be permanently deleted. There is no recovery.
</Callout>

### Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `project_id` | string (UUID) | Yes | Project ID to delete |
| `confirm_slug` | string | Yes | Project slug for confirmation (must match exactly) |

### Example

```javascript
// Delete a project (requires exact slug confirmation)
quoth_project_delete({
  project_id: "abc123...",
  confirm_slug: "my-project-slug"  // Must match exactly
})
```

### Safety

- ❌ Requires admin role
- ✅ Requires explicit slug confirmation
- ✅ Cascade deletes all project data
- ⚠️ **NO UNDO** - data is permanently lost

### What Gets Deleted

- All documents
- All embeddings and vector indexes
- All pending proposals
- All project memberships
- Project settings

## quoth_genesis

Bootstraps documentation for a new project. See the [Genesis Guide](/docs/guides/genesis) for detailed usage.

### Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `depth_level` | string | No | `minimal`, `standard`, or `comprehensive` |
| `focus` | string | No | `full_scan` or `update_only` |
| `language_hint` | string | No | Primary language hint |

## Prompts (Personas)

<Callout type="info">
  **Recommended:** Use `quoth_guidelines` tool instead of prompts. It provides the same rules with lower token cost (~150 tokens vs ~2,000) and Claude can call it autonomously.
</Callout>

For strict behavioral enforcement, Quoth still provides prompts that configure AI behavior for an entire conversation:

### quoth_architect

Enforces "Single Source of Truth" rules. Use before writing code:

```
/prompt quoth_architect
```

The AI will automatically search documentation before generating code.

### quoth_auditor

Reviews code against documentation. Use for code review:

```
/prompt quoth_auditor
```

Distinguishes between violations (bad code) and new patterns (needs documentation).

### quoth_documenter

Documents code as you build:

```
/prompt quoth_documenter
```

Fetches templates and creates structured documentation immediately.

## Quoth Plugin Hooks

When using Claude Code with Quoth connected, lightweight hooks provide automatic hints:

| Hook | Output | Tokens |
|------|--------|--------|
| SessionStart | "Quoth MCP active. Strongly recommend `quoth_guidelines('code')` and `quoth_search_index` before writing code." | ~25 |
| PreToolUse (Edit/Write) | "Quoth patterns available via `quoth_guidelines()` and `quoth_search_index`" | ~15 |
| Stop | Badge enforcement instruction | ~20 |

### Badge Format

When you use any `quoth_*` tools, end your response with:

```
┌─────────────────────────────────────────────────┐
│ 🪶 Quoth                                        │
│   ✓ patterns/testing-pattern.md (vitest mocks) │
└─────────────────────────────────────────────────┘
```

## Rate Limits

| Tier | Requests/min | Search/day |
|------|--------------|------------|
| Free | 10 | 100 |
| Pro | 60 | 1,000 |
| Enterprise | Unlimited | Unlimited |

<Callout type="tip">
  Rate limits apply per API key. Generate separate keys for different projects to avoid hitting limits.
</Callout>
